...
| DEF-010 | Medium | AC-FR-01-01 | PRD `docs/PRD_MagicSquare.md` §13.1 vs RED 테스트 계약 대조 | `grid=None` → `NULL_INPUT` / `입력이 null입니다` (AC-FR01-03) | 테스트는 `INVALID_SIZE` / `Grid must be 4x4.` 기대 | **요구사항·테스트 계약 불일치** (RED 스펙 vs PRD v1.1) | **RESOLVED (2026-05-29)**: AC-FR-01-01 슬라이스는 `INVALID_SIZE` + `Grid must be 4x4.` SSOT 유지 (`test_ac_fr_01_01`, `schemas.py`). `NULL_INPUT` 등 세분 코드는 FR-01-02~08 후속 GREEN에서 PRD §13.1과 함께 도입 |
